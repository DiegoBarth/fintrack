...
describe('useApiError', () => {
  ...
  describe('Critical Processing Failure (Catch Block)', () => {
    it('should handle catastrophic failure during error processing', () => {
      const { result } = renderHook(() => useApiError());

      const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => { });

      const explosiveError = new Error();
      Object.defineProperty(explosiveError, 'message', {
        get: () => {
          throw new Error('Explosão interna');
        },
      });

      result.current.handleError(explosiveError);
      ...
      expect(mockError).toHaveBeenCalledWith('Erro desconhecido');

      consoleSpy.mockRestore();
    });
  })
  describe('Branch Coverage Fallbacks', () => {
    it('should show "Erro desconhecido" when Error message is an empty string (Line 91)', () => {
      const { result } = renderHook(() => useApiError())
    ...
    it('should show default message when non-Error object converts to empty string (Line 100)', () => {
      const { result } = renderHook(() => useApiError())

      result.current.handleError('')

      expect(mockError).toHaveBeenCalledWith('Erro ao processar a requisição. Tente novamente')
    })
  })
})