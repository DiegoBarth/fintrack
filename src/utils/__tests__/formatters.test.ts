...
describe('formatters', () => {
  ...
  describe('formatCurrency', () => {
    ...
    it('should return R$ 0,00 when input has no numeric characters (Line 98)', () => {
      expect(formatCurrency('').replace(/\u00A0/g, ' ')).toBe('R$ 0,00')
      expect(formatCurrency('abc').replace(/\u00A0/g, ' ')).toBe('R$ 0,00')
      expect(formatCurrency('!@#$').replace(/\u00A0/g, ' ')).toBe('R$ 0,00')
    })
  })
  ...
  describe('getMonthAndYear', () => {
    ...
    it('should extract month and year from reference month string (YYYY-MM)', () => {
      const result = getMonthAndYearFromReference('2026-05');
    ...
    it('should handle months with two digits correctly', () => {
      const result = getMonthAndYearFromReference('2026-12');
      expect(result).toEqual({
        month: '12',
        year: '2026'
      });
    ...
    it('should create a date object in local timezone from ISO string', () => {
    ...
    it('should avoid UTC shifts by using local Date constructor', () => {
      const result = parseLocalDate('2026-01-01');
      expect(result.getHours()).toBe(0);
    });

    it('should extract month and year from ISO date string (YYYY-MM-DD)', () => {
      const result = getMonthAndYear('2026-05-20')
    ...
    it('should handle ISO string with different months', () => {
      const result = getMonthAndYear('2026-12-31')
