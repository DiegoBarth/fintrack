...
import { createCommitment, createCard, updateCommitment, deleteCommitment, payCardStatement } from '@/api/endpoints/commitment'
...
export function useCommitment(month: string, year: string) {
  ...
  const { data: allCommitments = [], isLoading, isError } = useQuery(getCommitmentsQueryOptions(year))

  const commitments = month === 'all'
    ? allCommitments
