...
describe('useExpense', () => {
  ...
  describe('Query: list expenses', () => {
    it('should fetch expenses for the year and filter by month', async () => {
      ...
      expect(expenseApi.listExpenses).toHaveBeenCalledWith('2026')
  ...
  describe('Edge Cases: Coverage for Missing Items in Cache', () => {
    it('should not call updateCacheAfterEditExpense if oldExpense is not found (Line 49)', async () => {
      const updatedExpense: Expense = {
        rowIndex: 999,
        description: 'Ghost',
        category: 'Outros',
      ...
      const queryClient = new QueryClient()
      queryClient.setQueryData(['expenses', '2026'], [])

      const wrapper = ({ children }: { children: React.ReactNode }) => (
        <QueryClientProvider client={queryClient}>{children}</QueryClientProvider>
      )

      const { result } = renderHook(() => useExpense('1', '2026'), { wrapper })

      await result.current.update({ rowIndex: 999, amount: 100 })

      expect(queryClient.getQueryData(['expenses', '2026'])).toEqual([])
    })

    it('should not call updateCacheAfterDeleteExpense if deletedExpense is not found (Line 68)', async () => {
      vi.mocked(expenseApi.listExpenses).mockResolvedValue([])
      vi.mocked(expenseApi.deleteExpense).mockResolvedValue(undefined as any)
      ...
      await result.current.remove(888)

      expect(expenseApi.deleteExpense).toHaveBeenCalledWith(888)
    })
  })
})